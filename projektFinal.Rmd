...
Prilikom izrade projekta, koristili smo se podatcima o dioničkim fondovima dobivenim iz sljedećeg skupa podataka. 
```{r fig.height=3.5, fig.width=5, echo=FALSE}
mutual_funds <- read.csv("mutual_funds.csv")
```
Osim metapodataka o fondovima, kao što su ime, izdavač i kategorija, svaki fond sadrži širok spektar atributa. U projektu nisu korišteni svi podatci, već se istraživanje fokusira na par ključnih podataka i njihovom povezanošću. Jedan od ključnih podataka vezan za istraživanje je povrat fonda. Budući da je uz skup podataka vezan cijeli niz atributa vezanih uz povrat, mi smo kao mjeru povrata koristili atribut "fund_return_10years", tj. povrat fonda u zadnjih 10 godina bez uzimanja u obzir rizik fonda. Stoga to treba uzeti u obzir prilikom daljnje analize rada.


## Strategije investiranja
Growth, value i blend su tri stila investiranja koje ćemo usporediti u ovom projektu.
Growth fondovi ulažu u dionice firmi za koje se pretpostavlja veliki rast.
Value fondovi investiraju u dionice čija je tržišna vrijednost ispod stvarne, odnosno u podcijenjene firme.
Blend investiranje je investiranje u različite dionice iz istog razreda, npr. S&P500. Možemo ih smatrati kombinacijom value i growth investiranja.
```{r  echo=FALSE}
godine <- c(2019, 2018, 2017, 2016, 2015, 2014, 2013, 2012, 2011, 2010)
#barplot(summary(na.omit(mutual_funds$investment)))
print('Growth')
...
plot(godine,c(value_investment),
     main = "Broj fondova po strategijima u zadnjih 10 godina",
     type = "l",
     frame = FALSE,
     ylim = c(1000, 3000),
     col="red")
...
plot(godine,value_return,
     main = "Povrati fondova po strategijima u zadnjih 10 godina",
     type = "l",
     frame = FALSE,
     col="red")
...
Kako bih saznali koji stil investiranja daje najbolje rezultate testirati ćemo njihove povrate na temelju podataka 1-godišnjeg,
...
cat("Podatci o povratu fondova sa strategijom Growth u zadnjih godinu dana\n")
summary(growth_1years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Value u zadnjih godinu dana\n")
summary(value_1years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Blend u zadnjih godinu dana\n")
summary(blend_1years)
cat("\n")

cat("Podatci o povratu fondova sa strategijom Growth u zadnje 3 godine\n")
summary(growth_3years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Value u zadnje 3 godine\n")
summary(value_3years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Blend u zadnje 3 godine\n")
summary(blend_3years)
cat("\n")

cat("Podatci o povratu fondova sa strategijom Growth u zadnjih 5 godina\n")
summary(growth_5years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Value u zadnjih 5 godina\n")
summary(value_5years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Blend u zadnjih 5 godina\n")
summary(blend_5years)
cat("\n")

cat("Podatci o povratu fondova sa strategijom Growth u zadnjih 10 godina\n")
summary(growth_10years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Value u zadnjih 10 godina\n")
summary(value_10years)
cat("\n")
cat("Podatci o povratu fondova sa strategijom Blend u zadnjih 10 godina\n")
...
hist(growth_1years,
     breaks = 20,
    main = 'Histogram jednogodišnjih povrata Growth fondova',
    xlab = 'Povrat u %',
    col='red')
hist(blend_1years,
     breaks = 20,
     main = 'Histogram jednogodišnjih povrata Blend fondova',
     xlab = 'Povrat u %',
     col='red')
hist(value_1years,
     breaks = 20,
     main = 'Histogram jednogodišnjih povrata Value fondova',
     xlab = 'Povrat u %',
     col='red')
...
hist(growth_3years,
     breaks = 20,
    main = 'Histogram trogodišnjih povrata Growth fondova',
    xlab = 'Povrat u %',
    col='red')
hist(blend_3years,
     breaks = 20,
     main = 'Histogram trogodišnjih povrata Blend fondova',
     xlab = 'Povrat u %',
     col='red')
hist(value_3years,
     breaks = 20,
     main = 'Histogram trogodišnjih povrata Value fondova',
     xlab = 'Povrat u %',
     col='red')
...
Growth investiranje daje najbolje povrate za 1, 3, 5 i 10-godišnje podatke. Sljedeći je Bland način investiranja,te naposljetku Value.


## Povrat fondova ovisno o stilu investiranja kada uzmemo rizik u obzir
Utvrdili smo koji stil investiranja je najbolji ako uzmemo u obzir samo povrat fondova. Međutim, tada zanemarujemo rizik koji određeni stil investiranja donosi sa sobom.
Sada ćemo testirati povrat fonda ako uzmemo u obzir i rizik. Varijabla koju ćemo koristiti je sharp ratio jer ona dosta dobro prikazuje povrat fonda naspram rizika. Sharp ratio je omjer povrata fonda i standardne devijacije.
...
U nastavku ćemo promatrati kategorije fondova. Pogledat ćemo uspijevaju li fondovi pobijediti kategoriju u smislu povrata.
Nakon toga ćemo podijeliti dataset u dva dijela. Za prvi dio ćemo uzeti fondove koji uspijevaju pobijediti svoju kategoriju, a za drugi dio one koji ne uspijevaju.
...
Pogledajmo u kojim kategorijama fondovi najviše pobjeđuju.
Iskoristit ćemo t-test za svaku kategoriju i prikazati postotak fondova koji su prošli test.
Gledamo  je li stvarna srednja vrijednost povrata fondova u 10 godina drugačija od povrata kategorije u 10 godina.
Pretpostavka $H_0$ je da fondovi ne pobjeđuju kategorije, tj. mean(fund_return_10years) < category_return_10years.
```{r echo=FALSE}
testovi = data.frame()
...
manji_invest = table(manji[manji$investment=="Blend" | manji$investment=="Growth" | manji$investment=="Value",]$investment)
tbl = rbind(veci_invest, manji_invest)
...
hist(mutual_funds$net_assets, main="Net assets [%]", xlab="Net assets", ylab="Count")
hist(mutual_funds$net_annual_expense_ratio_fund,main='Net annual expense ratio fund[%]', xlab='Expense ratio', ylab='Count')
hist(mutual_funds$net_annual_expense_ratio_category,main='Net annual expense ratio of category[%]', xlab='Expense ratio', ylab='Count')
hist(mutual_funds$fund_return_10years,main='Fund return in 10 years[%]', xlab='Return', ylab='Count')
hist(mutual_funds$category_return_10years,main='Category return in 10 years[%]', xlab='Return', ylab='Count')
mutual_funds <- mutual_funds %>% mutate(success = fund_return_10years - category_return_10years)
hist(mutual_funds$success,probability=T,main='Rate of success[%]', xlab='Success', ylab='Count',breaks = 20)
...
median <- median(mutual_funds_outliers$net_assets)
mutual_funds_large <- mutual_funds_outliers[mutual_funds_outliers$net_assets > median,]
mutual_funds_small <- mutual_funds_outliers[mutual_funds_outliers$net_assets <= median,]
...
```{r echo=FALSE, include=FALSE}
cat('Prosječni uspjeh skupljih fondova iznosi  ', mean(mutual_funds_outliers_expensive$success),'\n')
cat('Prosječni uspjeh jeftinijih fondova iznosi  ', mean(mutual_funds_outliers_cheap$success),'\n')
require(modeest)
...
boxplot(mutual_funds_outliers_expensive$success, mutual_funds_outliers_cheap$success, 
        ...
        main='Boxplot of expensive and cheap fund success rate')

hist(mutual_funds_outliers_expensive$success,
     breaks = 20,
     main='Histogram of rate of success of expensive funds',
     xlab='Success rate')

hist(mutual_funds_outliers_cheap$success, 
     main='Histogram of rate of success of cheap funds',
     xlab='Success rate')
...
```
Iz rezultata je vidljivo da se dobivene distribucije značajno razlikuju od normalne.Stoga nećemo koristiti t-test kako bismo usporedili srednje vrijednosti tih skupova, nego ćemo koristiti njegovu neparametarsku inačicu, tj. Mann-Whitney-Wilcoxov test.   
...
```
Iz dobivenog rezultata možemo očitati vrlo malu p-vrijednost, što nam govori da možemo odbaciti hipotezu H0 u korist H1. Drugim riječima, možemo reći da su jeftiniji fondovi u prosjeku uspješniji.


Varijabla "morningstar_rating" služit će nam kao druga mjera uspjeha fondova. Stoga ćemo analizirati i te podatke za sve skupove
...
cat("\nPrikaz rejtinga većih fondova\n")
table(mutual_funds_large$morningstar_rating)
cat("\nPrikaz  rejtinga manjih fondova\n")
...
cat("\nPrikaz frekvencija rejtinga većih fondova\n")
round(prop.table(large_rating),digits=3)
cat("\nPrikaz frekvencija rejtinga manjih fondova\n")
round(prop.table(small_rating),digits=3)
```

```{r, echo=FALSE}
barplot(summary(mutual_funds_outliers_expensive$morningstar_rating),main='Morningstar rating for expensive funds')
barplot(summary(mutual_funds_outliers_cheap$morningstar_rating),main='Morningstar rating for cheap funds')
...
```{r, fig.width = 14, fig.height=5, echo=FALSE, include=FALSE}
...
cat("Broj visoko rangiranih većih fondova:")
large_high_ratings
cat("Broj svih većih fondova:")
large_collective
...
cat("Broj visoko rangiranih manjih fondova:")
small_high_ratings
cat("Broj svih manjih fondova:")
small_collective
...
plot(mutual_funds_outliers$net_assets, mutual_funds_outliers$success)
```
...
fit.size <- lm(success~net_assets, data=mutual_funds_outliers)

plot(mutual_funds_outliers$net_assets, mutual_funds_outliers$success)
lines(mutual_funds_outliers$net_assets, fit.size$fitted.values, col='red')
```
...
```{r visestruka regresija, echo=FALSE}
...
```{r kategorijske ulazne varijable - sirove, echo=FALSE}
...
```{r visestruka regresija opet, echo=FALSE}
...
fit.multi = lm(success ~ net_annual_expense_ratio_fund + price_earnings + net_assets, mutual_funds_outliers)
